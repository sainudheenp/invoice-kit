...
export function emitBox(el: Element, s: CSSStyleDeclaration, ctx: WalkerCtx): void {
  ...
  for (let rectIndex = 0; rectIndex < rects.length; rectIndex++) {
    ...
    const bWidths       = sides.map(d => pxToPt((s as any)[`border${d}Width`] ?? '0px'))
    const bColorAlphas  = sides.map(d => parseColorAlpha((s as any)[`border${d}Color`] ?? ''))
    const bStyles       = sides.map(d => (s as any)[`border${d}Style`] as string ?? 'none')

    const allSame = bWidths.every(v => v === bWidths[0]) &&
      bStyles.every(v => v === bStyles[0]) &&
      bColorAlphas.every((c, i) => JSON.stringify(c) === JSON.stringify(bColorAlphas[0]))
    ...
    interface BgLayer { gradient?: Gradient; conicSrc?: Uint8Array; box: { dx: number; dy: number; w: number; h: number; radius?: BorderRadius } }
      ...
      if (uniformSolid && bWidths[0] > 0 && bStyles[0] !== 'none' && bColorAlphas[0]) {
        ...
        for (let i = 0; i < 4; i++) {
          ...
          if (bWidths[i] > 0 && bStyles[i] !== 'none' && bColorAlphas[i]) {
            ...
            ctx.commands.push({
              type: 'line', page, ...sideLine(i, bWidths[i] / 2),
              width: bWidths[i], color: lineColor!,
...
export function captureTextNode(
    ...
    if (clipsText) {
      if (s.backgroundImage && s.backgroundImage !== 'none') {
        for (const layer of splitByTopLevelComma(s.backgroundImage)) {
          ...
          if (g?.stops.length) { sub = [g.stops[0].color[0], g.stops[0].color[1], g.stops[0].color[2]]; break }
        }
      }
      if (!sub) {
  ...
  interface Deco {
    ...
    thicknessPt?: number; underlineOffsetPt?: number
  }
  const decos: Deco[] = []
  {
      ...
      if (dl && dl !== 'none') {
        for (const part of dl.split(' ')) {
          ...
          decos.push({
            ...
            thicknessPt:       thM ? Math.max(0.1, +thM[1] / PX_PER_PT) : undefined,
            underlineOffsetPt: uoM ? +uoM[1] / PX_PER_PT : undefined,
          })
        }
      }
