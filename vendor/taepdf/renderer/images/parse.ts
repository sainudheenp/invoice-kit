...
// Everything else is browser-decoded in the renderer (renderer/images/) and
...
// decodes those instead (renderer/images/decode.ts)
function parsePng(bytes: Uint8Array): ParsedImage | null {
  ...
  // output is computable BEFORE inflating — passing it as fflate's `out` caps
  // the decompression itself at that size (fflate truncates to fit rather than
  // growing an internal buffer to match whatever the stream claims), closing
  ...
  let raw: Uint8Array
  try { raw = unzlibSync(idat, { out: new Uint8Array(needed) }) } catch { return null }
