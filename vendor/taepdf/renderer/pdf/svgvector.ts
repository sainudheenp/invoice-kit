...
function parseSvgColor(v: string | null | undefined, currentColor: ColorAlpha): ColorAlpha | null {
  ...
  if (hex3) { const [r,g,b] = hex3[1]; return [parseInt(r+r,16), parseInt(g+g,16), parseInt(b+b,16), 255] }
...
function readStyle(el: Element, inherited: Style, currentColor: ColorAlpha): Style {
  ...
  return {
    ...
    strokeWidth:   swRaw !== undefined && swRaw !== null ? parseFloat(swRaw) : inherited.strokeWidth,
    fillOpacity:   foRaw !== undefined && foRaw !== null ? parseFloat(foRaw) : inherited.fillOpacity,
    strokeOpacity: soRaw !== undefined && soRaw !== null ? parseFloat(soRaw) : inherited.strokeOpacity,
    dash,
...
function bboxOfOps(ops: PathSeg[]): { x: number; y: number; w: number; h: number } {
  ...
  for (const seg of ops) {
    for (let i = 0; i < seg.args.length; i += 2) {
      minX = Math.min(minX, seg.args[i]);     maxX = Math.max(maxX, seg.args[i])
      minY = Math.min(minY, seg.args[i + 1]); maxY = Math.max(maxY, seg.args[i + 1])
    }
  }
