...
import {
  get_glyph_ids, shape_text, get_advance_widths, deflate,
...
export class PdfDoc {
  ...
  private byteLen       = 0
  private objectNumber  = 0
  private offsets:      number[] = [0]

  private fonts:        DocFont[] = []
  private fontMap:      Map<string, number> = new Map()
  private usedFonts:    Set<string> = new Set()

  private images:       EmbedImage[] = []

  private gradDefs:     GradDef[] = []
  private shadPats:     ShadPat[] = []
  private gradSoftMasks: GradSoftMask[] = []
  private extGStates:   { alpha: number; blend: string }[] = []

  private allPageBufs:  string[][] = []
  private pageAnnots:   PageAnnot[][] = []
  private currentPageIdx = -1
  private pageObjIds:   number[] = []
  private formFieldObjIds: number[] = []
  ...
  private rootDictObjId:     number
  private resourceDictObjId: number
  ...
  private namedDests:   [string, number, number][] = []

  private objStmQueue:    ObjStmItem[] = []
  private objStmMembers:  [number, number, number][] = []
  private captureStack:   string[][] = []

  private structRoot?: StructNode
  private pdfA         = false
  private pdfaLang?:    string

  private formatW: number
  private formatH: number
  ...
  private get ctx(): InternalCtx {
    return this as unknown as InternalCtx
  }
  ...
  private newObjectDeferred(): number {
    this.objectNumber++
  ...
  private outBytes(b: Uint8Array): void {
  ...
  private queueForObjStm(content: string): number {
    const oid = this.newObjectDeferred()
  ...
  private strLit(s: string): string {
    if (this.security) {
  ...
  private encBytes(data: Uint8Array): Uint8Array {
  ...
  private getOrCreateFont(name: string, style: string, weight: number): number {
  ...
  draw_path(
    ...
    stroke?: { color: [number, number, number]; width: number; dash?: number[]; lineCap?: number; lineJoin?: number },
  ): void {
  ...
  text(text: string, x: number, y: number, baseline: string,
    ...
    // deferred until after the fontIdx guard so an aborted draw never leaves
  ...
  private emitColorGlyphRun(
    ...
    for (let i = 0; i <= gids.length; i++) {
      ...
      if (!special) { if (runStart < 0) runStart = i; continue }
      if (runStart >= 0) {
      ...
      if (colrOf[i]) {
        ...
        if (imgId !== undefined && imgId !== 0xFFFFFFFF) {
