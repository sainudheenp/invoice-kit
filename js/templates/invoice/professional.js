...
window._INV_TEMPLATES.professional = function (d) {
  ...
  d.items.forEach(function (it, i) {
    ir += '<tr>' +
      '<td style="padding:8px 10px;text-align:center;color:#666;font-size:13px;border-bottom:1px solid #e0e0e0">' + (i+1) + '</td>' +
      '<td style="padding:8px 10px;font-size:13px;color:#333;border-bottom:1px solid #e0e0e0">' + esc(it.desc) + '</td>' +
      '<td style="padding:8px 10px;text-align:center;font-size:12px;color:#666;border-bottom:1px solid #e0e0e0">' + it.qty + '</td>' +
      '<td style="padding:8px 10px;text-align:right;font-size:13px;color:#666;border-bottom:1px solid #e0e0e0">' + (parseFloat(it.price) || 0).toFixed(d.dp) + '</td>' +
      '<td style="padding:8px 10px;text-align:right;font-size:14px;font-weight:700;color:#111;border-bottom:1px solid #e0e0e0">' + it.amount + '</td></tr>';
  });
  if (!ir) ir = '<tr><td colspan="5" style="text-align:center;color:#bbb;padding:24px;font-size:14px;font-style:italic">No items</td></tr>';
  ...
  return '<div style="font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#333;position:relative;background:#fff;line-height:1.7;min-height:100vh;padding:0;font-kerning:normal">' +
    ...
    (c.sub ? '<div style="font-size:14px;color:rgba(255,255,255,0.8);margin-top:2px">' + esc(c.sub) + '</div>' : '') +
    ...
    '<div style="font-size:11px;text-transform:uppercase;letter-spacing:1px;color:rgba(255,255,255,0.7)">Invoice</div>' +
    ...
    '<div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.8px;color:' + ac + ';margin-bottom:4px">Bill To</div>' +
    '<div style="font-weight:700;font-size:15px;color:#111">' + esc(d.cust||'---') + '</div>' +
    '<div style="font-size:13px;color:#666;line-height:1.5;margin-top:2px">' +
    [d.addr, d.ph && 'Tel: ' + d.ph, d.cr && 'CR: ' + d.cr, d.em].filter(Boolean).join('<br>') +
    '</div></div>' +
    '<div style="flex:0 0 210px;border:1px solid #e8e8e8;border-radius:6px;padding:10px 14px;font-size:13px;line-height:2">' +
    ...
    /* items table */
    '<table style="width:100%;border-collapse:collapse;font-size:13px;margin-bottom:4mm;border:1px solid #e0e0e0;border-radius:6px;overflow:hidden">' +
    '<thead><tr style="background:' + ac + '">' +
    '<th style="padding:8px 10px;text-align:center;font-weight:600;font-size:11px;color:#fff;text-transform:uppercase;letter-spacing:.3px">#</th>' +
    '<th style="padding:8px 10px;text-align:left;font-weight:600;font-size:11px;color:#fff;text-transform:uppercase;letter-spacing:.3px">Description</th>' +
    '<th style="padding:8px 10px;text-align:center;font-weight:600;font-size:11px;color:#fff;text-transform:uppercase;letter-spacing:.3px">Qty</th>' +
    '<th style="padding:8px 10px;text-align:right;font-weight:600;font-size:11px;color:#fff;text-transform:uppercase;letter-spacing:.3px">Price</th>' +
    '<th style="padding:8px 10px;text-align:right;font-weight:600;font-size:11px;color:#fff;text-transform:uppercase;letter-spacing:.3px">Amount</th>' +
    '</tr></thead><tbody>' + ir +
    '</tbody></table>' +
    ...
    /* notes & terms */
    (d.notes || c.invTerms ? '<div style="margin-bottom:4mm;padding:3mm 0;border-top:1px solid #eee;border-bottom:1px solid #eee;font-size:13px;color:#666">' +
    ...
    /* footer */
    '<div style="border-top:3px solid ' + pc + ';padding:3mm 14mm;display:flex;justify-content:space-between;font-size:11px;color:' + ac + '">' +
    '<div>' + esc(c.name) + (c.loc ? ' | ' + esc(c.loc) : '') + '</div>' +
    '<div>' + [c.tel && 'Tel: ' + c.tel, c.email].filter(Boolean).join(' | ') + '</div>' +
    '</div>' +
    (c.invFooter ? '<div style="padding:0 14mm 2mm;text-align:center;font-size:10px;color:#bbb">' + esc(c.invFooter) + '</div>' : '') +
    '</div>';
};
