...
window._REC_TEMPLATES.professional = function (d) {
  ...
  return '<div style="font-family:Arial,Helvetica,sans-serif;font-size:13px;color:#333;position:relative;background:#fff;line-height:1.6;min-height:100vh;padding:0;font-kerning:normal">' +
    ...
    (c.sub ? '<div style="font-size:12px;color:rgba(255,255,255,0.8);margin-top:1px">' + esc(c.sub) + '</div>' : '') +
    ...
    '<div style="font-size:9px;text-transform:uppercase;letter-spacing:1px;color:rgba(255,255,255,0.7)">Receipt</div>' +
    ...
    '<div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:' + ac + ';margin-bottom:2px">Received From</div>' +
    ...
    '<div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:' + ac + ';margin-bottom:2px">Date</div>' +
    ...
    '<tr style="background:#fafafa"><td style="padding:2.5mm 10px;font-size:10px;font-weight:700;color:' + ac + ';text-transform:uppercase;letter-spacing:.3px;width:25%;border-bottom:1px solid #eee">Payment</td>' +
    '<td style="padding:2.5mm 10px;font-size:13px;color:#222;border-bottom:1px solid #eee">' + esc(d.pm) + d.chqHtml + '</td></tr>' +
    ((d.bk||d.td) ? '<tr><td style="padding:2.5mm 10px;font-size:10px;font-weight:700;color:' + ac + ';text-transform:uppercase;letter-spacing:.3px;border-bottom:1px solid #eee">Bank / Date</td>' +
    '<td style="padding:2.5mm 10px;font-size:13px;color:#222;border-bottom:1px solid #eee">' + esc(d.bk) + (d.bk&&d.td?' / ':'') + esc(d.td) + '</td></tr>' : '') +
    '<tr style="background:#fafafa"><td style="padding:2.5mm 10px;font-size:10px;font-weight:700;color:' + ac + ';text-transform:uppercase;letter-spacing:.3px">Purpose</td>' +
    ...
    '<div style="font-size:9px;color:' + ac + ';margin-top:1px">Receiver</div>' +
    (c.signature ? '<div style="margin-top:4px"><img src="' + esc(c.signature) + '" style="max-width:80px;max-height:30px;object-fit:contain"><div style="font-size:9px;color:' + ac + '">Authorized Signature</div></div>' : '') +
    '</div></div>' : '') +
    '</div>' +
    '<div style="border-top:3px solid ' + pc + ';padding:2.5mm 14mm;display:flex;justify-content:space-between;font-size:9px;color:' + ac + '">' +
    '<div>' + esc(c.name) + (c.loc ? ' | ' + esc(c.loc) : '') + '</div>' +
    '<div>' + [c.tel && 'Tel: ' + c.tel, c.email].filter(Boolean).join(' | ') + '</div>' +
    '</div>' +
    (c.bankName ? '<div style="padding:0 14mm 1.5mm;text-align:center;font-size:8px;color:#bbb">' + esc(c.bankName) + (c.bankAcc?' | A/c: '+c.bankAcc:'') + (c.bankIban?' | IBAN: '+c.bankIban:'') + '</div>' : '') +
    '</div>';
};
