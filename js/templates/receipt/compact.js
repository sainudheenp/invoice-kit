...
window._REC_TEMPLATES.compact = function (d) {
  ...
  return '<div style="font-family:Arial,Helvetica,sans-serif;font-size:13px;color:#333;position:relative;background:#fff;line-height:1.5;min-height:100vh;padding:5mm 7mm 0;font-kerning:normal">' +
    ...
    '<div style="font-size:10px;color:#888">' + d.dt + '</div></div>' +
    '</div>' +
    '<div style="font-size:10px;color:#888;margin-bottom:2mm;padding:2px 0;border-top:1px solid #ddd;border-bottom:1px solid #ddd">' +
    [c.loc, c.tel && 'Tel: ' + c.tel, c.email].filter(Boolean).join(' | ') +
    '</div>' +
    '<table style="width:100%;border-collapse:collapse;font-size:12px;margin-bottom:3mm">' +
    '<tr><td style="padding:2mm;border-bottom:1px solid #e8e8e8;font-size:11px;font-weight:700;color:#888;text-transform:uppercase;width:25%">From</td>' +
    '<td style="padding:2mm;border-bottom:1px solid #e8e8e8;font-size:13px;font-weight:600;color:#111">' + esc(d.rf||'\u2014') + '</td></tr>' +
    '<tr><td style="padding:2mm;border-bottom:1px solid #e8e8e8;font-size:11px;font-weight:700;color:#888;text-transform:uppercase">Amount</td>' +
    '<td style="padding:2mm;border-bottom:1px solid #e8e8e8;font-size:13px;font-weight:700">' + d.amFmt + ' ' + d.cur.symbol + '</td></tr>' +
    '<tr><td style="padding:2mm;border-bottom:1px solid #e8e8e8;font-size:11px;font-weight:700;color:#888;text-transform:uppercase">Payment</td>' +
    '<td style="padding:2mm;border-bottom:1px solid #e8e8e8;font-size:13px">' + esc(d.pm) + d.chqHtml + '</td></tr>' +
    ((d.bk||d.td) ? '<tr><td style="padding:2mm;border-bottom:1px solid #e8e8e8;font-size:11px;font-weight:700;color:#888;text-transform:uppercase">Bank</td>' +
    '<td style="padding:2mm;border-bottom:1px solid #e8e8e8;font-size:13px">' + esc(d.bk) + (d.bk&&d.td?' / ':'') + esc(d.td) + '</td></tr>' : '') +
    '<tr><td style="padding:2mm;border-bottom:1px solid #e8e8e8;font-size:11px;font-weight:700;color:#888;text-transform:uppercase">Purpose</td>' +
    '<td style="padding:2mm;border-bottom:1px solid #e8e8e8;font-size:13px">' + esc(d.bg||'\u2014') + '</td></tr>' +
    '</table>' +
    '<div style="font-size:11px;color:#888;font-style:italic;margin-bottom:3mm;text-align:center">' + esc(d.ww) + '</div>' +
    ...
    '<div style="font-size:9px;color:#888">Receiver / Signature</div>' +
    '</div></div>' : '') +
    '<div style="position:absolute;bottom:3mm;left:7mm;right:7mm;border-top:1px solid #ddd;padding-top:1mm;display:flex;justify-content:space-between;font-size:9px;color:#888">' +
