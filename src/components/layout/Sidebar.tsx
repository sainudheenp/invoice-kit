    ...
    <>
      <aside className={`fixed top-0 left-0 h-full w-[248px] bg-[var(--color-side-bg)] flex flex-col z-50 transition-transform duration-300 max-md:-translate-x-full max-md:shadow-[var(--shadow-pop)] ${ui.sidebarOpen ? 'max-md:translate-x-0' : ''}`}>
        <div className="flex items-center gap-2.5 px-5 h-16 border-b border-[var(--color-border)] shrink-0">
          ...
          <span className="text-base font-bold text-[var(--color-text)] tracking-tight">invoice<span className="text-[var(--color-primary)]">kit</span></span>
        </div>

        <nav className="flex-1 p-3 overflow-y-auto space-y-5">
          {NAV_GROUPS.map((group) => (
            <div key={group.label}>
              <div className="px-3 pb-1.5 text-[10px] font-semibold uppercase tracking-widest text-[var(--color-text3)]">
                {group.label}
              </div>
                  ...
                  return (
                    <button
                      ...
                      className={`relative w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all duration-150 cursor-pointer ${
                        active
                          ? 'bg-[var(--color-side-active)] text-[var(--color-text)] font-semibold'
                          : 'text-[var(--color-side-text)] hover:bg-[var(--color-side-hover)] hover:text-[var(--color-text)]'
                      }`}
                    >
                      {active && (
        ...
        <div className="p-3 border-t border-[var(--color-border)] space-y-2">
          ...
          <button
            onClick={toggleDark}
            className="w-full flex items-center gap-3 px-3 py-2 rounded-xl text-sm text-[var(--color-side-text)] hover:bg-[var(--color-side-hover)] hover:text-[var(--color-text)] transition-colors cursor-pointer"
          >
            <span className="w-8 h-8 rounded-lg bg-[var(--color-side-hover)] flex items-center justify-center text-[var(--color-primary)]">
          ...
          <button
            ...
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl bg-[var(--color-side-hover)] hover:bg-[var(--color-side-active)] transition-colors cursor-pointer"
          >
            ...
            <span className="min-w-0 text-left">
              <span className="block text-xs font-semibold text-[var(--color-text)] truncate">{co?.name || 'No company'}</span>
              <span className="block text-[10px] text-[var(--color-text3)]">
