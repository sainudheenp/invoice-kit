...
export function ReceiptBeirak(d: RecTemplateData) {
  ...
  const itemsRows = d.items.length > 0 ? d.items.map((item, i) => `
    <tr${i % 2 === 1 ? ` style="background:${LB}"` : ''}>
      <td style="padding:8px 12px;border:1px solid ${GR};text-align:center;color:#4b5563">${i + 1}</td>
      <td style="padding:8px 12px;border:1px solid ${GR};color:#1f2937">${esc(item.desc)}</td>
      <td style="padding:8px 12px;border:1px solid ${GR};text-align:center;color:#4b5563">${item.qty}</td>
      <td style="padding:8px 12px;border:1px solid ${GR};text-align:right;color:#4b5563">${d.cur.symbol}${item.price.toFixed(d.dp)}</td>
  ...
  return watermarkWrap(`
    <div style="font-family:'Inter',Arial,Helvetica,sans-serif;font-size:14px;line-height:1.5;color:#374151;position:relative;background:#fff;min-height:100vh;padding:0">
      ...
      <div style="padding:0 16mm;display:flex;justify-content:flex-end">
        <table style="width:240px;border-collapse:collapse;font-size:13px">
          <tr style="background:${DB};color:#fff">
            <td style="padding:10px 14px;border:1px solid ${DB};font-weight:600">Amount</td>
            <td style="padding:10px 14px;border:1px solid ${DB};text-align:right;font-weight:700;font-size:18px">${d.cur.symbol}${d.amFmt}</td>
          </tr>
        </table>
      </div>
      ${d.ww ? `<div style="padding:4mm 16mm 0;font-size:12px;color:#6b7280;font-style:italic;text-align:right">${esc(d.ww)}</div>` : ''}
      ${d.bg ? `<div style="padding:2mm 16mm;font-size:13px;color:#4b5563"><strong style="color:#111827">Purpose:</strong> ${esc(d.bg)}</div>` : ''}
      <div style="padding:0 16mm;display:flex;gap:24px;margin-top:8mm;align-items:flex-end">
        <div style="flex:1">
          ${d.rv ? `<div style="border-top:2px solid ${DB};width:160px;padding-top:6px;font-size:13px;color:#374151;text-align:center">${esc(d.rv)}</div><div style="font-size:12px;color:#6b7280;text-align:center;margin-top:4px">Receiver</div>` : ''}
        </div>
        ...
        <div style="flex:1;text-align:right">
          ${d.sg ? `<div style="border-top:2px solid ${DB};width:160px;padding-top:6px;margin-left:auto;font-size:13px;color:#374151;text-align:center">${esc(d.sg)}</div><div style="font-size:12px;color:#6b7280;text-align:center;margin-top:4px">Signatory</div>` : ''}
        </div>
      </div>
