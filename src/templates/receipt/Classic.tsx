    ...
    <div style="font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#2d2d2d;position:relative;background:#fff;min-height:100vh">
      ...
      <div style="padding:6mm 16mm">
        <div style="display:flex;justify-content:space-between;align-items:center">
          <div>
            <div style="font-size:16px;font-weight:700;color:${pc}">RECEIPT VOUCHER</div>
      ...
      <div style="padding:6mm 16mm;display:flex;gap:20px;align-items:flex-end">
        ...
        ${d.rv ? `<div><div style="border-top:1px solid #999;width:120px;padding-top:2px;font-size:12px;text-align:center">${esc(d.rv)}</div><div style="font-size:11px;color:#666;text-align:center">Receiver${showAr ? ' / \u0627\u0644\u0645\u0633\u062A\u0644\u0645' : ''}</div></div>` : ''}
        ${c.signature ? `<div><img src="${esc(c.signature)}" style="max-width:100px;max-height:50px" /><div style="font-size:11px;color:#666;text-align:center">Authorized Signature${showAr ? ' / \u0627\u0644\u062A\u0648\u0642\u064A\u0639' : ''}</div></div>` : ''}
        ${d.sg ? `<div><div style="border-top:1px solid #999;width:120px;padding-top:2px;font-size:12px;text-align:center">${esc(d.sg)}</div><div style="font-size:11px;color:#666;text-align:center">Signatory</div></div>` : ''}
      </div>
      ...
      ${c.bankName ? `<div style="position:absolute;bottom:7mm;left:16mm;right:16mm;font-size:11px;color:#666">${[c.bankName, c.bankAcc, c.bankIban].filter(Boolean).join(' | ')}</div>` : ''}
    </div>
  `, d.comp.watermark)
}
