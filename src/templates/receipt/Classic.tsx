...
export function ReceiptClassic(d: RecTemplateData) {
  ...
  const itemsRows = d.items.length > 0 ? d.items.map((item, i) => `
    <tr${altRow(i)}>
      <td style="padding:10px 16px;color:#6b7280;border-bottom:1px solid #e5e7eb">${i + 1}</td>
      <td style="padding:10px 16px;border-bottom:1px solid #e5e7eb;color:#1f2937" colspan="2">${esc(item.desc)}</td>
      <td style="padding:10px 16px;text-align:right;direction:ltr;border-bottom:1px solid #e5e7eb;color:#4b5563">${item.qty} x ${d.cur.symbol}${item.price.toFixed(d.dp)}</td>
      <td style="padding:10px 16px;text-align:right;font-weight:600;border-bottom:1px solid #e5e7eb;color:#111827">${d.cur.symbol}${item.amount.toFixed(d.dp)}</td>
    </tr>
  `).join('') : ''
    ...
    <div style="font-family:'Inter',Arial,Helvetica,sans-serif;font-size:14px;line-height:1.5;color:#374151;position:relative;background:#fff;min-height:100vh;padding:0">
      ...
      <div style="padding:8mm 20mm">
        <div style="display:flex;justify-content:space-between;align-items:flex-end">
          <div>
            ...
            ${showAr ? '<div dir="rtl" unicode-bidi="embed" style="font-size:16px;color:#9ca3af;margin-top:4px">\u0633\u0646\u062F \u0642\u0628\u0636</div>' : ''}
          </div>
      ...
      <div style="padding:4mm 20mm">
        <table style="width:100%;border-collapse:separate;border-spacing:0;font-size:13px">
          <thead>
            <tr>
              <th style="padding:10px 16px;text-align:left;border-bottom:2px solid #e5e7eb;color:#94a3b8;font-weight:600">#</th>
              <th style="padding:10px 16px;text-align:left;border-bottom:2px solid #e5e7eb;color:#94a3b8;font-weight:600" colspan="2">Description</th>
              <th style="padding:10px 16px;text-align:right;border-bottom:2px solid #e5e7eb;color:#94a3b8;font-weight:600">Rate</th>
              <th style="padding:10px 16px;text-align:right;border-bottom:2px solid #e5e7eb;color:#94a3b8;font-weight:600">Amount</th>
            </tr>
          </thead>
      ...
      <div style="padding:4mm 20mm">
        <table style="width:100%;border-collapse:collapse;font-size:13px">
          ${detailRows.map((row, i) => `
            <tr${altRow(i)}>
              <td style="padding:12px 16px;width:25%;font-weight:600;color:#111827;border-bottom:1px solid #f3f4f6">${row[0]}</td>
              <td style="padding:12px 16px;color:#374151;border-bottom:1px solid #f3f4f6">${row[1]}</td>
              ${showAr ? `<td style="padding:12px 16px;width:20%;direction:rtl;unicode-bidi:embed;text-align:right;font-size:15px;color:#9ca3af;border-bottom:1px solid #f3f4f6">${row[2]}</td>` : ''}
            </tr>
          `).join('')}
        </table>
      </div>
      <div style="padding:8mm 20mm;display:flex;justify-content:flex-end">
        <div style="display:flex;align-items:flex-end;gap:16px;background:#f8fafc;padding:16px 28px;border-radius:8px;border:1px solid #e5e7eb">
          ...
          <div style="text-align:center;padding:0 16px">
            ...
            <div style="font-size:22px;font-weight:600;color:#4b5563">${String(d.fr).padStart(d.dp === 3 ? 3 : 2, '0')}</div>
          </div>
        </div>
      </div>
      ...
      ${c.bankName ? `<div style="position:absolute;bottom:13mm;left:20mm;right:20mm;font-size:12px;color:#9ca3af;text-align:center">${[c.bankName, c.bankAcc, c.bankIban].filter(Boolean).join(' | ')}</div>` : ''}
    </div>
  `, d.comp.watermark)
}
