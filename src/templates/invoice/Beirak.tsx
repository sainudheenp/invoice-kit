...
export function InvoiceBeirak(d: InvTemplateData): string {
  ...
  const rows = d.items.map((item, i) => {
    ...
    <tr${i % 2 === 1 ? ` style="background:${LB};"` : ''}>
      ...
      <td style="padding:5px 8px;border:1px solid #c5ced9;font-size:10px;text-align:right;">${(item.taxRate || 0) > 0 ? item.taxRate + '%' : '-'}</td>
      <td style="padding:5px 8px;border:1px solid #c5ced9;font-size:10px;text-align:right;">${(item.taxRate || 0) > 0 ? d.cur.symbol + taxAmt.toFixed(d.dp) : '-'}</td>
