...
export function QuotationMinimal(d:QuotTemplateData): string {
  ...
  const rows = d.items.map((item, i) => `
    <tr>
      <td style="padding:3.75px 0;font-size:11.25px;color:#475569;text-align:center;">${i + 1}</td>
      <td style="padding:3.75px 0;font-size:11.25px;color:#334155;">${esc(item.desc)}</td>
      <td style="padding:3.75px 0;font-size:11.25px;color:#475569;text-align:right;">${item.qty}</td>
      <td style="padding:3.75px 0;font-size:11.25px;color:#475569;text-align:right;">${d.cur.symbol}${item.price.toFixed(d.dp)}</td>
      <td style="padding:3.75px 0;font-size:11.25px;color:#334155;text-align:right;font-weight:500;">${d.cur.symbol}${item.amount.toFixed(d.dp)}</td>
    </tr>
  `).join('')
...
<html><head><meta charset="utf-8"><style>
  ...
  * { box-sizing:border-box;margin:0;padding:0; }
  ...
  th { font-size:8.75px; color:#94a3b8; font-weight:500; padding:3.75px 0; border-bottom:1.25px solid #e2e8f0; text-align:left; text-transform:uppercase; letter-spacing:1px; }
...
${d.notes || d.terms ? `<div class="section">
  ${d.notes ? `<div class="section-title">Notes</div><div>${esc(d.notes)}</div>` : ''}
  ${d.terms ? `<div style="margin-top:5px;"><span class="section-title">Terms</span><br>${esc(d.terms)}</div>` : ''}
</div>` : ''}
