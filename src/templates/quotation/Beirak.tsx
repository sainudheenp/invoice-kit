...
export function QuotationBeirak(d: QuotTemplateData) {
  const c = d.comp
  const itemsRows = d.items.map((item, i) => `
    <tr${i % 2 === 1 ? ` style="background:${LB}"` : ''}>
      <td style="padding:10px 14px;border-bottom:1px solid ${GR};border-right:1px solid ${GR};border-left:1px solid ${GR};text-align:center;color:#4b5563">${i + 1}</td>
      <td style="padding:10px 14px;border-bottom:1px solid ${GR};border-right:1px solid ${GR};color:#1f2937">${esc(item.desc)}</td>
      <td style="padding:10px 14px;border-bottom:1px solid ${GR};border-right:1px solid ${GR};text-align:center;color:#4b5563">${item.qty}</td>
      <td style="padding:10px 14px;border-bottom:1px solid ${GR};border-right:1px solid ${GR};text-align:right;color:#4b5563">${d.cur.symbol}${item.price.toFixed(d.dp)}</td>
      <td style="padding:10px 14px;border-bottom:1px solid ${GR};border-right:1px solid ${GR};text-align:right;font-weight:600;color:#111827">${d.cur.symbol}${item.amount.toFixed(d.dp)}</td>
    </tr>
  `).join('')

  return watermarkWrap(`
    <div style="font-family:'Inter',Arial,Helvetica,sans-serif;font-size:13px;line-height:1.5;color:#374151;position:relative;background:#fff;min-height:100vh;padding:0">
      <div style="text-align:center;padding:10mm 16mm 6mm">
        ...
        ${c.sub ? `<div style="font-size:14px;color:#4b5563;margin-top:4px">${esc(c.sub)}</div>` : ''}
      </div>
      <div style="text-align:center;margin:4mm 0 6mm">
        <div style="display:inline-block;background:${LB};border:2px solid ${DB};border-radius:8px;padding:8px 24px">
          <div style="font-size:15px;font-weight:700;color:${RD};letter-spacing:2px">QUOTATION</div>
        </div>
      </div>
      ...
      <div style="padding:0 16mm;display:flex;gap:6mm">
        <table style="width:50%;border-collapse:collapse;font-size:13px">
          ${[
            ['Quotation No.', esc(d.no)],
            ['Date', d.dt],
            ['Valid Until', d.validDt],
          ].map(([l, v]) => `
            <tr>
      ...
      <div style="padding:6mm 16mm">
        <table style="width:100%;border-collapse:collapse;font-size:13px">
          ...
          <tbody>
            ${itemsRows || '<tr><td colspan="5" style="padding:40px;text-align:center;color:#9ca3af;border:1px solid #e5e7eb">No items</td></tr>'}
          </tbody>
        </table>
      </div>
      <div style="padding:0 16mm;display:flex;justify-content:flex-end">
        <table style="width:280px;border-collapse:collapse;font-size:13px">
          <tr style="background:${LB}"><td style="padding:10px 14px;border:1px solid ${GR};font-weight:600;color:${DB}">Subtotal</td><td style="padding:10px 14px;border:1px solid ${GR};text-align:right;color:#1f2937">${d.cur.symbol}${d.sv}</td></tr>
          ${d.disc > 0 ? `<tr><td style="padding:10px 14px;border:1px solid ${GR};font-weight:600;color:${RD}">Discount</td><td style="padding:10px 14px;border:1px solid ${GR};text-align:right;color:${RD}">-${d.cur.symbol}${d.dv}</td></tr>` : ''}
          ${d.vp > 0 ? `<tr style="background:${LB}"><td style="padding:10px 14px;border:1px solid ${GR};font-weight:600;color:${DB}">VAT (${d.vp}%)</td><td style="padding:10px 14px;border:1px solid ${GR};text-align:right;color:#1f2937">${d.cur.symbol}${d.vv}</td></tr>` : ''}
          <tr style="background:${DB};color:#fff">
      ...
      <div style="padding:0 16mm;margin-top:6mm;display:grid;gap:6px;font-size:12px;color:#4b5563">
      ...
      <div style="padding:0 16mm;display:flex;gap:24px;margin-top:8mm;align-items:flex-end">
        <div style="flex:1">
          ...
          <div style="border-top:2px solid ${DB};width:160px;padding-top:6px;font-size:13px;color:#374151">Accounts Department</div>
        </div>
        ...
        <div style="flex:1;text-align:right">
          ...
          <div style="border-top:2px solid ${DB};width:160px;padding-top:6px;margin-left:auto;font-size:13px;color:#374151">${esc(c.name)}</div>
        </div>
      </div>
