...
export function QuotationBeirak(d: QuotTemplateData) {
  ...
  return watermarkWrap(`
    <div style="font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#333;position:relative;background:#fff;min-height:100vh;padding:0">
      ...
      ${d.notes ? `<div style="padding:0 12mm;font-size:12px"><strong>Notes:</strong> ${esc(d.notes)}</div>` : ''}
      ${d.terms ? `<div style="padding:0 12mm;font-size:12px"><strong>Terms:</strong> ${esc(d.terms)}</div>` : ''}
      ...
      <div style="border-top:1px solid ${DB};margin:6mm 12mm 0;padding-top:3mm;font-size:11px;color:#666;text-align:center">
        QUOTATION | ${esc(c.name)}${c.tel ? ` | ${esc(c.tel)}` : ''}${c.email ? ` | ${esc(c.email)}` : ''}
      </div>
