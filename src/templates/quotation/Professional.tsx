...
export function QuotationProfessional(d:QuotTemplateData): string {
  ...
  const rows = d.items.map((item, i) => `
    <tr>
      <td style="padding:5px 7.5px;border:1.25px solid #cbd5e1;font-size:11.25px;text-align:center;">${i + 1}</td>
      <td style="padding:5px 7.5px;border:1.25px solid #cbd5e1;font-size:11.25px;">${esc(item.desc)}</td>
      <td style="padding:5px 7.5px;border:1.25px solid #cbd5e1;font-size:11.25px;text-align:right;">${item.qty}</td>
      <td style="padding:5px 7.5px;border:1.25px solid #cbd5e1;font-size:11.25px;text-align:right;">${d.cur.symbol}${item.price.toFixed(d.dp)}</td>
      <td style="padding:5px 7.5px;border:1.25px solid #cbd5e1;font-size:11.25px;text-align:right;">${d.cur.symbol}${item.amount.toFixed(d.dp)}</td>
    </tr>
  `).join('')
...
<html><head><meta charset="utf-8"><style>
  ...
  * { box-sizing:border-box;margin:0;padding:0; }
  body { font-family:'Courier New','Courier','Lucida Sans Typewriter',monospace; color:#1e293b; background:#fff; width:794px; padding:40px 50px; }
  ...
  th { background:#f1f5f9; color:#475569; font-size:10px; padding:5px 7.5px; text-align:left; font-weight:bold; text-transform:uppercase; letter-spacing:0.625px; border:1.25px solid #cbd5e1; }
  ...
  .notes-box { margin-top:15px; padding:10px 12.5px; border:1.25px solid #cbd5e1; font-size:11.25px; color:#475569; }
  ...
  .footer { margin-top:20px; padding-top:10px; border-top:1.25px solid #cbd5e1; font-size:8.75px; color:#64748b; text-align:center; text-transform:uppercase; }
</style></head><body>
...
${d.notes || d.terms ? `<div class="notes-box">
  ${d.notes ? `<div style="margin-bottom:2.5px;"><strong>Notes:</strong> ${esc(d.notes)}</div>` : ''}
